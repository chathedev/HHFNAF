...
async function getContent() {
  try {
    const filePath = path.join(process.cwd(), "public", "content", "kontakt.json")
    const raw = await fs.readFile(filePath, "utf-8")
    return JSON.parse(raw)
  } catch (err) {
    console.error("Failed to load kontakt content, using default:", err)
    return DEFAULT_CONTENT
  }
}

// Reading the content server-side (instead of the old client-side fetch-after-mount)
// means the page never has an empty "content is null" state to show a loading
// spinner for — it arrives already resolved in the initial render.
